...
public class VectorObserver_Date_long_
  implements Vector_Date_long_,
             Changing,
             Dumpable,
             ManagedResource
{
    protected Vector_Date_long_ observed_vector;
    ...
    public VectorObserver_Date_long_ (CallContext context, Vector_Date_long_ observed_vector,VectorObserverEventPolicy event_policy, ModificationType modification_type) {
        ...
        if (this.observed_vector != null && modification_type != ModificationType.STATE_LOAD) {
            if (getEventPolicy(context) == VectorObserverEventPolicy.ON_DEMAND) {
                ...
                for (Date item : observed_vector.getIterable_Date_(context)) {
                    onAdd(context, item);
                }
            }
        }
    ...
    public VectorObserver_Date_long_ (CallContext context, Vector_Date_long_ observed_vector,VectorObserverEventPolicy event_policy) {
        this(context, observed_vector, event_policy, null);
    }

    public VectorObserver_Date_long_(CallContext context, Vector_Date_long_ observed_vector) {
        this(context, observed_vector, VectorObserverEventPolicy.ON_DEMAND);   
    }

    public void setObserved (CallContext context, Vector_Date_long_ observed_vector) {
        if (this.getObserved(context) != null) {
            for (Date item : this.observed_vector.getIterable_Date_(context)) {
    ...
    public Vector_Date_long_ getObserved (CallContext context) {
    ...
    protected void onAdd(CallContext context, Date item) {
    }

    protected void onRemove(CallContext context, Date item) {
    }
    ...
    public Date                                    set             (CallContext context, long index, Date item) {
        Date old_item = this.getObserved(context).set(context, index, item);
        if (old_item != null) { 
    ...
    public Date                                    replace         (CallContext context, long index, Date item) throws DoesNotExist {
        Date old_item = this.getObserved(context).replace(context, index, item);
        onRemove(context, old_item);
        ChangeEvent event = new RemoveEvent(context,old_item,index);   
        notify(context,event);
        onAdd(context, item);
    ...
    public Date                                    unset           (CallContext context, long index) {
        Date old_item = this.getObserved(context).unset(context, index);
        if (old_item != null) { 
           onRemove(context, old_item); 
           ChangeEvent event = new RemoveEvent(context,old_item,index);   
           notify(context,event);
    ...
    public Date                                    remove          (CallContext context, long index) throws DoesNotExist {
        Date old_item = this.getObserved(context).remove(context, index);
        onRemove(context, old_item);
        ChangeEvent event = new RemoveEvent(context,old_item,index);   
        notify(context,event);
        return old_item;
    ...
    public java.util.Iterator<Date> getIterator_Date_ (CallContext context) {
        return this.getObserved(context).getIterator_Date_(context);
    }

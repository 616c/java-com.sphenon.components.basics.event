...
public class VectorObserver_String_long_
  implements Vector_String_long_,
             Changing,
             Dumpable,
             ManagedResource
{
    protected Vector_String_long_ observed_vector;
    ...
    public VectorObserver_String_long_ (CallContext context, Vector_String_long_ observed_vector,VectorObserverEventPolicy event_policy, ModificationType modification_type) {
        ...
        if (this.observed_vector != null && modification_type != ModificationType.STATE_LOAD) {
            if (getEventPolicy(context) == VectorObserverEventPolicy.ON_DEMAND) {
                ...
                for (String item : observed_vector.getIterable_String_(context)) {
                    onAdd(context, item);
                }
            }
        }
    ...
    public VectorObserver_String_long_ (CallContext context, Vector_String_long_ observed_vector,VectorObserverEventPolicy event_policy) {
        this(context, observed_vector, event_policy, null);
    }

    public VectorObserver_String_long_(CallContext context, Vector_String_long_ observed_vector) {
        this(context, observed_vector, VectorObserverEventPolicy.ON_DEMAND);   
    }

    public void setObserved (CallContext context, Vector_String_long_ observed_vector) {
        if (this.getObserved(context) != null) {
            for (String item : this.observed_vector.getIterable_String_(context)) {
    ...
    public Vector_String_long_ getObserved (CallContext context) {
    ...
    protected void onAdd(CallContext context, String item) {
    }

    protected void onRemove(CallContext context, String item) {
    }
    ...
    public String                                    set             (CallContext context, long index, String item) {
        String old_item = this.getObserved(context).set(context, index, item);
        if (old_item != null) { 
    ...
    public String                                    replace         (CallContext context, long index, String item) throws DoesNotExist {
        String old_item = this.getObserved(context).replace(context, index, item);
        onRemove(context, old_item);
        ChangeEvent event = new RemoveEvent(context,old_item,index);   
        notify(context,event);
        onAdd(context, item);
    ...
    public String                                    unset           (CallContext context, long index) {
        String old_item = this.getObserved(context).unset(context, index);
        if (old_item != null) { 
           onRemove(context, old_item); 
           ChangeEvent event = new RemoveEvent(context,old_item,index);   
           notify(context,event);
    ...
    public String                                    remove          (CallContext context, long index) throws DoesNotExist {
        String old_item = this.getObserved(context).remove(context, index);
        onRemove(context, old_item);
        ChangeEvent event = new RemoveEvent(context,old_item,index);   
        notify(context,event);
        return old_item;
    ...
    public java.util.Iterator<String> getIterator_String_ (CallContext context) {
        return this.getObserved(context).getIterator_String_(context);
    }

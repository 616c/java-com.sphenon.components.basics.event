// instantiated with javainst.pl from /workspace/sphenon/projects/components/basics/many/v0001/origin/source/java/com/sphenon/basics/many/templates/VectorObserver.javatpl
...
public class VectorObserver_Object_long_
  implements Vector_Object_long_,
             ...
             , Anchorable
{
    protected Vector_Object_long_ observed_vector;
    ...
    public VectorObserver_Object_long_ (CallContext context, Vector_Object_long_ observed_vector,VectorObserverEventPolicy event_policy, ModificationType modification_type) {
        ...
        if (this.observed_vector != null && modification_type != ModificationType.STATE_LOAD) {
            if (getEventPolicy(context) == VectorObserverEventPolicy.ON_DEMAND) {
                ...
                for (Object item : observed_vector.getIterable_Object_(context)) {
                    onAdd(context, item);
                }
            }
        }
    ...
    public VectorObserver_Object_long_ (CallContext context, Vector_Object_long_ observed_vector,VectorObserverEventPolicy event_policy) {
        this(context, observed_vector, event_policy, null);
    }

    public VectorObserver_Object_long_(CallContext context, Vector_Object_long_ observed_vector) {
        this(context, observed_vector, VectorObserverEventPolicy.ON_DEMAND);   
    }

    public void setObserved (CallContext context, Vector_Object_long_ observed_vector) {
        if (this.getObserved(context) != null) {
            for (Object item : this.observed_vector.getIterable_Object_(context)) {
    ...
    public Vector_Object_long_ getObserved (CallContext context) {
    ...
    protected void onAdd(CallContext context, Object item) {
    }

    protected void onRemove(CallContext context, Object item) {
    }
    ...
    public Object                                    set             (CallContext context, long index, Object item) {
        Object old_item = this.getObserved(context).set(context, index, item);
        if (old_item != null) { 
    ...
    public Object                                    replace         (CallContext context, long index, Object item) throws DoesNotExist {
        Object old_item = this.getObserved(context).replace(context, index, item);
        onRemove(context, old_item);
        ChangeEvent event = new RemoveEvent(context,old_item,index);   
        notify(context,event);
        onAdd(context, item);
    ...
    public Object                                    unset           (CallContext context, long index) {
        Object old_item = this.getObserved(context).unset(context, index);
        if (old_item != null) { 
           onRemove(context, old_item); 
           ChangeEvent event = new RemoveEvent(context,old_item,index);   
           notify(context,event);
    ...
    public Object                                    remove          (CallContext context, long index) throws DoesNotExist {
        Object old_item = this.getObserved(context).remove(context, index);
        onRemove(context, old_item);
        ChangeEvent event = new RemoveEvent(context,old_item,index);   
        notify(context,event);
        return old_item;
    ...
    public java.util.Iterator<Object> getIterator_Object_ (CallContext context) {
        return this.getObserved(context).getIterator_Object_(context);
    }
